...
import { useMTableToolbar } from '../../store/hooks';

export function MTableToolbarWithRedux(props) {
  ...
  const state = useSelector((state) => state.toolbar);
  const {
    setSearchText,
    setColumnsButtonAnchorEl,
    setExportButtonAnchorEl
  } = useMTableToolbar();

  const onSearchChange = (searchText) => {
    setSearchText(searchText, (text) => props.onSearchChanged(text));
  };
  ...
  function renderSearch() {
    const localization = {
      ...MTableToolbarWithRedux.defaultProps.localization,
      ...props.localization
    };
    if (props.search) {
      return (
          ...
          <span>
            <Tooltip title={localization.showColumnsTitle}>
              <IconButton
                color="inherit"
                onClick={
                  (event) => setColumnsButtonAnchorEl(event.currentTarget)
                  // setState({
                  //   ...state,
          ...
          <span>
            <Tooltip title={localization.exportTitle}>
              <IconButton
                color="inherit"
                onClick={
                  (event) => setExportButtonAnchorEl(event.currentTarget)
                  // setState({
                  //   ...state,
...
MTableToolbarWithRedux.defaultProps = {
  actions: [],
  columns: [],
...
MTableToolbarWithRedux.propTypes = {
...
const MTableToolbarRef = React.forwardRef(function MTableToolbarRef(
  ...
  return <MTableToolbarWithRedux {...props} forwardedRef={ref} />;
});
